...
check "run -i stdin" "found" \
    "$(printf 'from-stdin\n' | timeout "$T" "$MVM" run -i alpine cat | tr -d '\r' | grep -q from-stdin && echo found)"
...
if command -v script >/dev/null 2>&1; then
    ...
    RAW_OUT=$(printf 'exit\n' | run_pty timeout "$T" "$MVM" run -it alpine sh -c '[ -t 0 ] && echo RAW-TTY-OK' 2>/dev/null)
    ...
    check "run -t emits crlf" "found" \
        "$(timeout "$T" "$MVM" run -t alpine printf 'A\n' | grep -q "$(printf 'A\r')" && echo found)"
    ...
    run_pty timeout "$T" "$MVM" run -it alpine sh \
